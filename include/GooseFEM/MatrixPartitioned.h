...
private:
    ...
    {
        // Assertions for input consistency for *this specific batch*
        ...
        for (size_t e = 0; e < nelem; ++e) {
            for (size_t m = 0; m < nnodes_per_elem; ++m) {
                for (size_t i = 0; i < m_ndim; ++i) {
                    ...
                    size_t global_dof_m_i = m_dofs(conn_elem(e, m), i); 
                    size_t partitioned_dof_m_i = m_part1d(global_dof_m_i); 

                    for (size_t n = 0; n < nnodes_per_elem; ++n) {
                        for (size_t j = 0; j < m_ndim; ++j) {

                            size_t global_dof_n_j = m_dofs(conn_elem(e, n), j); 
                            size_t partitioned_dof_n_j = m_part1d(global_dof_n_j);

                            double elem_stiff_val = elemmat(e, m * m_ndim + i, n * m_ndim + j);

                            if (partitioned_dof_m_i < m_nnu && partitioned_dof_n_j < m_nnu) {
                                m_Tuu.push_back(Eigen::Triplet<double>(
                                    partitioned_dof_m_i, partitioned_dof_n_j, elem_stiff_val
                                ));
                            }
                            else if (partitioned_dof_m_i < m_nnu) {
                                m_Tup.push_back(Eigen::Triplet<double>(
                                    partitioned_dof_m_i, partitioned_dof_n_j - m_nnu, elem_stiff_val
                                ));
                            }
                            else if (partitioned_dof_n_j < m_nnu) {
                                m_Tpu.push_back(Eigen::Triplet<double>(
                                    partitioned_dof_m_i - m_nnu, partitioned_dof_n_j, elem_stiff_val
                                ));
                            }
                            else {
                                m_Tpp.push_back(Eigen::Triplet<double>(
                                    partitioned_dof_m_i - m_nnu,
                                    partitioned_dof_n_j - m_nnu,
                                    elem_stiff_val
                                ));
                            }
                        }
