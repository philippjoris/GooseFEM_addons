...
template <size_t N>
class CohesiveBilinear2d : public GMatTensor::Cartesian2d::Array<N> {
protected:
...
// 
                ...
                xt::xtensor_fixed<double, xt::xshape<m_ndim, m_ndim>> Temp;
                GT::A2_dot_B2(P_T.data(), C_local.data(), Temp.data());
...
// --- CohesiveExponential2d class ---

template <size_t N>
class CohesiveExponential2d : public GMatTensor::Cartesian2d::Array<N> {
protected:
    array_type::tensor<double, N> m_beta; ///< weighting for tangential separation per item.
    array_type::tensor<double, N> m_Kn; ///< initial normal separation stiffness per item.
    array_type::tensor<double, N> m_Kt; ///< initial tangential separation stiffness per item.
...
template <size_t N>
class CohesiveBilinear3d : public GMatTensor::Cartesian3d::Array<N> {
protected:
    array_type::tensor<double, N> m_beta; ///< weighting for tangential separation per item.
    array_type::tensor<double, N> m_Kn; ///< initial normal separation stiffness per item.
    array_type::tensor<double, N> m_Kt; ///< initial tangential separation stiffness per item.    
    ...
    using GMatTensor::Cartesian3d::Array<N>::m_stride_tensor1;
    ...
    using GMatTensor::Cartesian3d::Array<N>::m_shape_tensor1;
...
public:
    ...
    */
    ...
    )
    {
        ...
        // Initialize initial stiffness matrix explicitly
        for (size_t i = 0; i < m_size; ++i) {
            auto C_local = xt::adapt(&m_C_local.flat(i * m_ndim * m_ndim), {m_ndim, m_ndim});
            auto C = xt::adapt(&m_C.flat(i * m_ndim * m_ndim), {m_ndim, m_ndim});

            C_local.fill(0.0);
            C_local(0, 0) = m_Kn.flat(i);
            for (size_t k = 0; k < m_ndim - 1; ++k) {
                C_local(k + 1, k + 1) = m_Kt.flat(i);
            }

            // Assuming P_matrix is set to identity initially:
            namespace GT = GMatTensor::Cartesian3d::pointer;
            array_type::tensor<double, 2> P_matrix_i = GMatTensor::Cartesian3d::I2();
            xt::xtensor_fixed<double, xt::xshape<m_ndim, m_ndim>> P_T_local;
            ...
            for (size_t k = 0; k < m_ndim; k++)
            {
                ...
                {
                    P_T_local(k,l) = P_matrix_i(l,k);
                }
            }
            
            xt::xtensor_fixed<double, xt::xshape<m_ndim, m_ndim>> Temp;
            GT::A2_dot_B2(C_local.data(), P_T_local.data(), Temp.data());
            GT::A2_dot_B2(Temp.data(), P_matrix_i.data(), C.data());
        }
    }
    ...
    #pragma omp for
            for (ptrdiff_t i = 0; i < (ptrdiff_t)m_size; ++i) {
                ...
                if (delta_n < 0.0) { 
                    ...
                    if (!compute_tangent) {
                        GT::A2_dot_B1(P_T.data(), T_local.data(), T.data()); 
                        continue;
                    }
                    ...
                    if (current_D_v < 0.0 + 1e-6) {
                    ...
                    else if (current_D_v >= 1.0 - 1e-6) {
                        C_local.fill(0.0);
                    }
                    else {
                        // Correctly calculate individual derivatives for each component
                        double d_delta_eff_d_delta_n = (delta_eff > 1e-12) ? delta_n / delta_eff : 0.0;
                        double d_dv_d_delta_n = d_current_D_v_d_D_instant_trial * dD_d_delta_eff * d_delta_eff_d_delta_n;
                        ...
                        for (ptrdiff_t k = 1; k < (ptrdiff_t)m_ndim; ++k) {
                            ...
                            // Fill off-diagonal terms involving the normal component
                            C_local(0, k) = -Kn * delta(0) * d_dv_d_delta_tk;
                            C_local(k, 0) = -Kt * delta(k) * d_dv_d_delta_n;

                            // Fill diagonal tangential terms
                            ...
                            for (ptrdiff_t l = 1; l < (ptrdiff_t)m_ndim; ++l) {
                                if (k != l) {
                                    ...
                                    C_local(k, l) = -Kt * delta(k) * d_dv_d_delta_tl;
                                }
                            }
                        }
                    }
