...
public:
     ...
     */
    template <class T, class T_Conn>
    void assemble(const T& elemmat, const T_Conn& conn)
    {
        GOOSEFEM_ASSERT(conn.shape()[0] == elemmat.shape()[0] &&
                    "Number of elements in 'conn' must match number of elements in 'elemmat'.");
        derived_cast().assemble_impl(elemmat, conn);
    }

    void finalize(bool stabilize)
    {
        derived_cast().finalize_impl(stabilize);
    }    

    void stabilize()
    {
        derived_cast().stabilize_impl();
    }      
...
public:
     ...
     */
    Matrix(const array_type::tensor<size_t, 2>& dofs)
    {
        m_dofs = dofs;
...
private:
    ...
    )
    {
        ...
        GOOSEFEM_ASSERT(elemmat.shape()[0] == nelem && "elemmat: Number of elements mismatch.");
        ...
        GOOSEFEM_ASSERT(conn_elem.dimension() == 2 && "Connectivity 'conn_elem' must be a 2D array (num_elements x num_nodes_per_elem).");
                ...
                for (size_t i = 0; i < m_ndim; ++i) {
                    for (size_t n = 0; n < nnodes_per_elem; ++n) { 
                        for (size_t j = 0; j < m_ndim; ++j) {
                            ...
                            size_t global_dof_row = m_dofs(global_node_id_m, i); 

                            size_t global_node_id_n = conn_elem(e, n);
                            size_t global_dof_col = m_dofs(global_node_id_n, j); 
    ...
    void stabilize_impl() {
        ...
        for (int i = 0; i < m_A.rows(); ++i) {
            if (std::abs(m_A.coeff(i, i)) < 1e-12) {
                m_A.coeffRef(i, i) += spring_val;
            }
        }
    }
